...
export type EngagementPlanProgram = {
  id: number;
...
export type ListResponse = { message: string; data: EngagementPlanProgram[] };

/** ---------- Global store (กัน hot-reload เคลียร์ค่า) ---------- */
type Store = {
  data: EngagementPlanProgram[];
  autoId: number;
};
...
declare global {
  var __ENGAGEMENT_PLAN_STORE__: Store | undefined;
}

const store: Store = globalThis.__ENGAGEMENT_PLAN_STORE__ ?? {
  data: structuredClone(initialData),
  autoId: Math.max(...initialData.map((x) => x.id)),
};

globalThis.__ENGAGEMENT_PLAN_STORE__ = store;
...
export function listPrograms(fiscalYear?: number): EngagementPlanProgram[] {
...
export function addProgram(input: Omit<EngagementPlanProgram, "id">) {
  const item: EngagementPlanProgram = { ...clone(input), id: ++store.autoId };
...
export function updateProgram(
  id: number,
  patch: Partial<Omit<EngagementPlanProgram, "id">>
) {
