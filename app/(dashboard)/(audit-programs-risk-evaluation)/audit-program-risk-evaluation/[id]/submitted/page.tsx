...
export default function SubmittedRiskAssessmentPage() {
  ...
  const detail = useMemo((): AuditProgramRiskEvaluation | null => {
    console.log('=== BUILDING SUBMITTED DETAIL OBJECT ===');
    ...
    // ดึงข้อมูลปัจจัยเสี่ยงจาก localStorage
        ...
        if (savedAssessments) {
          ...
          activityRisks = activityRisks.map(risk => {
            ...
            const risks_assessment = relatedAssessments.map((assess: Record<string, unknown>, index: number) => ({
              id: (assess.factorId as number) * 100 + ((assess.subFactorIndex as number) || index),
